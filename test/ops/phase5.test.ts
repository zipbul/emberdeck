...
describe('getCardTree', () => {
  ...
  it('builds a 3-level tree with correct structure', async () => {
    tc = await createTestContext();
    await createCard(tc.ctx, { key: 'root', summary: 'Root', type: 'architecture' });
  ...
  it('truncates at maxDepth and sets truncated=true only when children exist', async () => {
    tc = await createTestContext();
    await createCard(tc.ctx, { key: 'r', summary: 'Root', type: 'architecture' });
  ...
  it('leaf nodes: empty children, no truncated flag', async () => {
    tc = await createTestContext();
    await createCard(tc.ctx, { key: 'leaf-root', summary: 'Root', type: 'architecture' });
  ...
  it('caps maxDepth at 20', async () => {
    tc = await createTestContext();
    await createCard(tc.ctx, { key: 'cap-root', summary: 'Root', type: 'architecture' });
  ...
  it('includes type and status in tree nodes', async () => {
    tc = await createTestContext();
    await createCard(tc.ctx, { key: 'typed-root', summary: 'Root', type: 'architecture' });
    const tree = getCardTree(tc.ctx, 'typed-root');
    expect(tree.type).toBe('architecture');
  ...
  it('starting from a non-root (middle) node shows only its subtree', async () => {
    tc = await createTestContext();
    await createCard(tc.ctx, { key: 'top', summary: 'Top', type: 'architecture' });
  ...
  it('relations are not traversed, only parent/child hierarchy', async () => {
    tc = await createTestContext();
    await createCard(tc.ctx, { key: 'tr-root', summary: 'Root', type: 'architecture' });
...
describe('findCardsBySymbol boundary matching', () => {
  ...
  it('boundary match: matchType="boundary"', async () => {
    tc = await createTestContext();
    insertCard('spec/api', { boundary: ['src/api/**'] });
    const result = await findCardsBySymbol(tc.ctx, 'handleRequest', 'src/api/handler.ts');
    expect(result).toHaveLength(1);
    expect(result[0]!.matchType).toBe('boundary');
    expect(result[0]!.card.key).toBe('spec/api');
  });

  it('codeLink takes priority over boundary (no duplicate)', async () => {
    tc = await createTestContext();
    insertCard('spec/both', { boundary: ['src/auth/**'] });
    tc.ctx.codeLinkRepo.replaceForCard('spec/both', [
      { kind: 'function', file: 'src/auth/login.ts', symbol: 'login' },
    ]);
    const result = await findCardsBySymbol(tc.ctx, 'login', 'src/auth/login.ts');
    expect(result).toHaveLength(1);
    expect(result[0]!.matchType).toBe('codeLink');
  });

  it('boundary matching requires filePath', async () => {
    tc = await createTestContext();
    insertCard('spec/boundary-only', { boundary: ['src/**'] });
    const result = await findCardsBySymbol(tc.ctx, 'someSymbol');
    expect(result).toHaveLength(0);
  });

  it('non-matching boundary is not included', async () => {
    tc = await createTestContext();
    insertCard('spec/other', { boundary: ['src/other/**'] });
    const result = await findCardsBySymbol(tc.ctx, 'fn', 'src/api/route.ts');
    expect(result).toHaveLength(0);
  });

  it('multiple boundary patterns: matches if any pattern hits', async () => {
    tc = await createTestContext();
    insertCard('spec/multi', { boundary: ['src/db/**', 'src/api/**'] });
    const result = await findCardsBySymbol(tc.ctx, 'query', 'src/api/query.ts');
    expect(result).toHaveLength(1);
    expect(result[0]!.matchType).toBe('boundary');
  });

  it('same card matches one symbol via codeLink, different symbol via boundary', async () => {
    tc = await createTestContext();
    insertCard('spec/mixed', { boundary: ['src/handler/**'] });
    tc.ctx.codeLinkRepo.replaceForCard('spec/mixed', [
      { kind: 'function', file: 'src/auth.ts', symbol: 'login' },
    ]);

    // 'login' matched by codeLink
    const r1 = await findCardsBySymbol(tc.ctx, 'login', 'src/auth.ts');
    expect(r1).toHaveLength(1);
    expect(r1[0]!.matchType).toBe('codeLink');

    // 'dispatch' not in codeLinks, but file matches boundary
    const r2 = await findCardsBySymbol(tc.ctx, 'dispatch', 'src/handler/dispatch.ts');
    expect(r2).toHaveLength(1);
    expect(r2[0]!.matchType).toBe('boundary');
  });

  it('multiple cards with overlapping boundaries: all matching cards returned', async () => {
    tc = await createTestContext();
    insertCard('spec/broad', { boundary: ['src/**'] });
    insertCard('spec/narrow', { boundary: ['src/api/**'] });
    const result = await findCardsBySymbol(tc.ctx, 'fn', 'src/api/handler.ts');
    expect(result).toHaveLength(2);
    const keys = result.map((r) => r.card.key);
    expect(keys).toContain('spec/broad');
    expect(keys).toContain('spec/narrow');
    expect(result.every((r) => r.matchType === 'boundary')).toBe(true);
  });
});
