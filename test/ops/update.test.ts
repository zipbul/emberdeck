...
describe('updateCard', () => {
  ...
  it('should update status in DB when updateCardStatus is called', async () => {
    tc = await createTestContext();
    await createCard(tc.ctx, { key: 'st-card', summary: 'Status', type: 'brief', body: BRIEF_BODY, brief: makeTestBrief() });
  ...
  it('should update status in file frontmatter when updateCardStatus is called', async () => {
    tc = await createTestContext();
    await createCard(tc.ctx, { key: 'st-file', summary: 'Status file', type: 'brief', body: BRIEF_BODY, brief: makeTestBrief() });
  ...
  it('should update DB row and file when updateCardStatus is called while DB row is missing', async () => {
    tc = await createTestContext();
    await createCard(tc.ctx, { key: 'st-no-db', summary: 'No DB', type: 'brief', body: BRIEF_BODY, brief: makeTestBrief() });
  ...
  it('should reflect latest status after multiple status transitions', async () => {
    tc = await createTestContext();
    await createCard(tc.ctx, { key: 'multi-st', summary: 'Status', type: 'brief', body: BRIEF_BODY, brief: makeTestBrief() });
