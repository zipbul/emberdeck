...
describe('syncSpecAnnotations', () => {
  ...
  it('should not create duplicate link when link already exists', async () => {
    ...
    await createCard(tc.ctx, {
      ...
      summary: 'Dup',
      type: 'spec' as const });
    setCardCodeLinks(tc.ctx, 'dup-link', [{ kind: 'function', file: 'src/a.ts', symbol: 'existing' }]);
    tc.ctx.gildash = createMockGildash({
      searchAnnotations: () => [
  ...
  it('should preserve existing manual links when adding new annotation link', async () => {
    ...
    await createCard(tc.ctx, {
      ...
      summary: 'Preserve',
      type: 'spec' as const });
    setCardCodeLinks(tc.ctx, 'preserve', [{ kind: 'class', file: 'src/old.ts', symbol: 'OldClass' }]);
    tc.ctx.gildash = createMockGildash({
      searchAnnotations: () => [
...
describe('syncSymbolChanges', () => {
  ...
  it('should update symbol name on rename', async () => {
    ...
    await createCard(tc.ctx, {
      ...
      summary: 'Rename',
      type: 'spec' as const });
    setCardCodeLinks(tc.ctx, 'rename-card', [{ kind: 'function', file: 'src/auth.ts', symbol: 'oldName' }]);
    tc.ctx.gildash = createMockGildash({
      getSymbolChanges: () => [{
        changeType: 'renamed',
        symbolName: 'newName',
  ...
  it('should update file path on move', async () => {
    ...
    await createCard(tc.ctx, {
      ...
      summary: 'Move',
      type: 'spec' as const });
    setCardCodeLinks(tc.ctx, 'move-card', [{ kind: 'class', file: 'src/old-path.ts', symbol: 'MyClass' }]);
    tc.ctx.gildash = createMockGildash({
      getSymbolChanges: () => [{
        changeType: 'moved',
        symbolName: 'MyClass',
  ...
  it('should report removed symbols as broken without deleting links', async () => {
    ...
    await createCard(tc.ctx, {
      ...
      summary: 'Deleted',
      type: 'spec' as const });
    setCardCodeLinks(tc.ctx, 'del-card', [{ kind: 'function', file: 'src/gone.ts', symbol: 'deletedFn' }]);
    tc.ctx.gildash = createMockGildash({
      getSymbolChanges: () => [{
        changeType: 'removed',
  ...
  it('should handle rename where oldName is null (uses symbolName as fallback)', async () => {
    ...
    await createCard(tc.ctx, {
      ...
      summary: 'Fallback',
      type: 'spec' as const });
    setCardCodeLinks(tc.ctx, 'fallback', [{ kind: 'function', file: 'src/f.ts', symbol: 'sameName' }]);
    tc.ctx.gildash = createMockGildash({
      getSymbolChanges: () => [{
        changeType: 'renamed',
        symbolName: 'sameName',
  ...
  it('should handle multiple changes affecting the same card', async () => {
    ...
    await createCard(tc.ctx, {
      ...
      summary: 'Multi',
      type: 'spec' as const });
    setCardCodeLinks(tc.ctx, 'multi-change', [
...
describe('getLinkCoverage', () => {
  ...
  it('should return full coverage when all links resolve', async () => {
    ...
    await createCard(tc.ctx, {
      ...
      type: 'spec' as const });
    setCardCodeLinks(tc.ctx, 'full-cov', [
      { kind: 'function', file: 'src/a.ts', symbol: 'fnA' },
  ...
  it('should report broken links and reduced coverage', async () => {
    ...
    await createCard(tc.ctx, {
      ...
      summary: 'Partial',
      type: 'spec' as const });
    setCardCodeLinks(tc.ctx, 'partial-cov', [
      { kind: 'function', file: 'src/a.ts', symbol: 'exists' },
      { kind: 'function', file: 'src/c.ts', symbol: 'missing' },
    ]);
    tc.ctx.gildash = createMockGildash({
      getSymbolsByFile: (file: any) => {
  ...
  it('should find unreferenced symbols in linked files', async () => {
    ...
    await createCard(tc.ctx, {
      ...
      type: 'spec' as const });
    setCardCodeLinks(tc.ctx, 'unref', [{ kind: 'function', file: 'src/a.ts', symbol: 'fnA' }]);
    tc.ctx.gildash = createMockGildash({
  ...
  it('should handle searchSymbols returning non-array (error Result)', async () => {
    ...
    await createCard(tc.ctx, {
      ...
      type: 'spec' as const });
    setCardCodeLinks(tc.ctx, 'err-search', [{ kind: 'function', file: 'src/a.ts', symbol: 'x' }]);
    tc.ctx.gildash = createMockGildash({
  ...
  it('should count all links as broken when none resolve', async () => {
    ...
    await createCard(tc.ctx, {
      ...
      summary: 'All broken',
      type: 'spec' as const });
    setCardCodeLinks(tc.ctx, 'all-broken', [
      { kind: 'function', file: 'src/a.ts', symbol: 'fn1' },
      { kind: 'function', file: 'src/b.ts', symbol: 'fn2' },
    ]);
    tc.ctx.gildash = createMockGildash({
      searchSymbols: () => [],
      getSymbolsByFile: () => [],
