...
describe('CLI flag overrides matrix', () => {
  ...
  test('--config <path> picks up alternate config file', async () => {
    ...
    expect(existsSync(join(tmp, 'alt-cards/p.card.md'))).toBe(true);
    expect(existsSync(join(tmp, 'default-cards/p.card.md'))).toBe(false);
  });
  ...
  test('--dir overrides cardsDir from config', async () => {
    ...
    expect(r.exitCode).toBe(0);
    expect(existsSync(join(tmp, 'cli-cards/p.card.md'))).toBe(true);
    expect(existsSync(join(tmp, 'config-cards/p.card.md'))).toBe(false);
  });
