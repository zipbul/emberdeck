...
describe('buildCardPath', () => {
  // HP
  it('should return "/cards/api.md" when cardsDir="/cards" and slug="api"', () => {
    // Arrange
    ...
    // Assert
    expect(result).toBe('/cards/api.md');
  });

  it('should return "/base/specs/api.md" when nested slug given', () => {
    // Arrange
    const cardsDir = '/base';
    ...
    const result = buildCardPath(cardsDir, slug);
    // Assert
    expect(result).toBe('/base/specs/api.md');
  });
});
