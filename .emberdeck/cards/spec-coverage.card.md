...
- WHEN suggestCardScope is called, THEN uncovered symbols MUST be grouped by directory, and suggestions MUST include type (intent for multi-file dirs, spec for single-file), parent (nearest ancestor with a card), boundary pattern, and matching glossary words.
