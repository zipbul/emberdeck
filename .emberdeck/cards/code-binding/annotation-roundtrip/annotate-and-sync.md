...
spec:
  ...
  failures:
    - violation: gildash index unavailable.
      behavior: >-
        Each operation returns a transient-failure status; partial results from
        successful project queries are still returned.
    - violation: An annotation references a card key with no matching card row.
      behavior: >-
        ...
        `unmatched` array entry of the spec sync result; no cache row is written
        for it.
---
