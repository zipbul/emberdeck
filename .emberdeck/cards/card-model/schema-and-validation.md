...
brief:
  context:
    problem: >
      ...
      bulk-sync of on-disk

      markdown. Without strict validation the system accepts cards whose parent
      type violates the

      four-tier rule, whose required fields are missing, or whose internal
      cross-references

      (covers, governs, verifies, binds, derives) point at items that do not
      exist on the same card,

      producing downstream drift, broken cards, and corrupted hierarchy.
    impact:
      - statement: >-
      ...
      - statement: >-
          ...
          criteria, spec has codeLinks); a single generic validator is
  ...
  scope:
    goals:
      ...
      - id: G-002
        statement: >-
          Verify intra-card cross references (covers, governs, verifies, binds,
    ...
    non_goals:
      - id: NG-001
        statement: >-
          Validating that codeLink files or symbols actually exist (delegated to
          code-binding).
      - id: NG-002
        statement: >-
  ...
  design:
    ...
    components:
      ...
      - name: spec-refs-validator
        responsibility: >-
          Resolves spec.preconditions/postconditions/invariants binds against
          declared codeLinks and derives format.
        interacts_with: []
    data_flow:
