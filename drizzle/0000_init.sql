-- Emberdeck schema v2 (clean creation, no migration from v1)
...
--> statement-breakpoint
CREATE INDEX `idx_card_parent` ON `card` (`parent`);
--> statement-breakpoint
CREATE TABLE `tag` (
...
--> statement-breakpoint
CREATE UNIQUE INDEX `uq_card_relation` ON `card_relation` (`src_card_key`, `dst_card_key`, `is_reverse`);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE UNIQUE INDEX `uq_code_link` ON `code_link` (`card_key`, `kind`, `file`, `symbol`);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE INDEX `idx_changelog_card` ON `card_changelog` (`card_key`);
--> statement-breakpoint
